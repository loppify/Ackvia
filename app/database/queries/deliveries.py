...
from app.database.models import Delivery, Submission, Form, WorkspaceMembership


async def get_accessible_delivery_by_id(db: AsyncSession, delivery_id: int, user_id: uuid.UUID):
    return await db.scalar(
        select(Delivery)
        .join(Submission, Delivery.submission_id == Submission.id)
        .join(Form, Form.id == Submission.form_id)
        .join(WorkspaceMembership, WorkspaceMembership.workspace_id == Form.workspace_id)
