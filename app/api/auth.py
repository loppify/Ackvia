...
from app.services.auth import register_user, UserAlreadyExistsError, create_session, login_user, \
    InvalidCredentialsError, get_user_by_session_token, logout_session

router = APIRouter(prefix="/api/auth", tags=["auth"])


@router.post("/register", response_model=UserRead, status_code=status.HTTP_201_CREATED)
async def register_handler(user_cred: RegisterRequest, response: Response, db: AsyncSession = Depends(get_db)):
    try:
    ...
    response.set_cookie(value=session_token,
                        key="ackvia_session",
                        httponly=True,
                        secure=True,
                        samesite="lax",
...
async def login_handler(user_cred: RegisterRequest, response: Response, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_current_user(ackvia_session: str | None = Cookie(default=None),
...
async def logout_handler(res: Response, ackvia_session: str | None = Cookie(default=None),
                         db: AsyncSession = Depends(get_db)):
    if ackvia_session is not None:
        await logout_session(db, ackvia_session)

    res.delete_cookie(key="ackvia_session",
                      httponly=True,
                      secure=True,
                      samesite="lax",
                      path="/")
